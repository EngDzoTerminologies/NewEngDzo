...
public class DatabaseHelperFinancial extends SQLiteOpenHelper {
   ...
   // private static String DBPOLITICAL_NAME="eng_dictionary.db";
    ...
    FinancialActivity com = new FinancialActivity();


    public DatabaseHelperFinancial(Context context) {
    ...
    {
        Cursor c= myDataBase.rawQuery("SELECT acronym,speech,dzo_word,en_word,image,dzo_definition,category FROM financial WHERE en_word==('"+text+"') OR dzo_word=('"+text+"')",null);
        return c;
    }

    public Cursor getSuggestions(String text)
    {
        SharedPreferences sharedPreferences = myContext.getSharedPreferences("financialpreference", 0);
        if( sharedPreferences.getInt("financialSwitchValue", 1) == 1)
        {
            Cursor c =  myDataBase.rawQuery("SELECT _id, en_word FROM financial WHERE en_word LIKE '"+text+"%' LIMIT 40",null);
            return c;
        }
        else
        {
            Cursor c = myDataBase.rawQuery("SELECT _id,dzo_word FROM financial WHERE dzo_word LIKE '" + text + "%' LIMIT 40", null);
            return c;
        }
    }
    ...
    {
        myDataBase.execSQL("INSERT INTO history(financial) VALUES(UPPER('"+text+"'))");
