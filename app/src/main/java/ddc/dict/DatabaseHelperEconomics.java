...
public class DatabaseHelperEconomics extends SQLiteOpenHelper {
    ...
    EconomicsActivity com = new EconomicsActivity();

    public DatabaseHelperEconomics(Context context) {
    ...
    {
        Cursor c= myDataBase.rawQuery("SELECT speech,dzo_word,en_word,images,category FROM economics WHERE en_word==('"+text+"') OR dzo_word == ('"+text+"')" ,null);
        return c;
    }
    ...
    public Cursor getSuggestions(String text)
    {
        ...
        SharedPreferences sharedPreferences = myContext.getSharedPreferences("economicspreference", 0);
        if( sharedPreferences.getInt("economicsSwitchValue", 1) == 1)
        {
            Cursor c = myDataBase.rawQuery("SELECT _id, en_word FROM economics WHERE en_word LIKE '"+text+"%' LIMIT 40",null);
            return c;
        }
        else
        {
            Cursor c = myDataBase.rawQuery("SELECT _id,dzo_word FROM economics WHERE dzo_word LIKE '" + text + "%' LIMIT 40", null);
            return c;
        }
...
//        Cursor c= myDataBase.rawQuery("select value_en from bookmark",null);
        Cursor c= myDataBase.rawQuery("select distinct value_en,value_dzo from bookmark order by _id desc",null);
        return c;
    }
